    ...
    async () => {
        try {
            const response = await axios.get(
                "https://itnews-api.herokuapp.com/api/v1/post/drafts"
            )
...
export const fetchPostAccess1 = createAsyncThunk(
    ...
    async () => {
        try {
            const response = await axios.get(
                "https://itnews-api.herokuapp.com/api/v1/post/public"
            )
            if (response.status === 200) {
                return await { ...response.data, status: response.status }
            }
...
export const fetchPostAccess2 = createAsyncThunk(
    ...
    async () => {
        try {
            const response = await axios.get(
                "https://itnews-api.herokuapp.com/api/v1/post/unlisted"
            )
            if (response.status === 200) {
                return await { ...response.data, status: response.status }
            }
...
export const fetchPostNewest = createAsyncThunk(
    ...
    async () => {
        try {
            const response = await axios.get(
                "https://itnews-api.herokuapp.com/api/v1/post/trending"
            )
            if (response.status === 200) {
                return await { ...response.data, status: response.status }
            }
    ...
    async (data) => {
        try {
            const response = await axios.put(
                `https://itnews-api.herokuapp.com/api/v1/post/${data.id_post}/access/${data.access}`
            )
            if (response.status === 200) {
                return await {
                    ...response.data,
                ...
                if (action.payload.status === 200) {
                    if (action.payload.type === 0) {
                        state.postUser = state.postUser.filter(
                            (post) =>
                                post.post.id_post !== action.payload.id_post
                        )
