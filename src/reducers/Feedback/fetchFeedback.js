...
export const fetchFeed = createAsyncThunk("feedback/fetch", async () => {
    try {
        const response = await axios.get(
            `https://itnews-api.herokuapp.com/api/v1/feedback/unread`
        )
    ...
    async (id_feedback) => {
        try {
            const response = await axios.put(
                `https://itnews-api.herokuapp.com/api/v1/feedback/${id_feedback}/read`
            )
            if (response.status === 200) {
                return await {
                    ...response.data,
    ...
    async (id_feedback) => {
        try {
            const response = await axios.delete(
                `https://itnews-api.herokuapp.com/api/v1/feedback/${id_feedback}`
            )
            if (response.status === 200) {
                return await {
                    ...response.data,
            ...
            .addCase(readFeed.fulfilled, (state, action) => {
                if (action.payload.status === 200) {
                    state.feedback = state.feedback.filter(
                        (feedback) =>
                            feedback.id_feedback !== action.payload.id_feedback
                    )
                } else {
