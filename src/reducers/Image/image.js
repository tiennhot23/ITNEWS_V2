...
export const fetchImage = createAsyncThunk("image/fetch", async () => {
    try {
        const response = await axios.get(
            `https://itnews-api.herokuapp.com/api/v1/image/account`
        )
...
export const updateImage = createAsyncThunk("image/update", async (data) => {
    try {
        const response = await axios.patch(
            `https://itnews-api.herokuapp.com/api/v1/image/change/${data.id_image}`,
            data.image
        )
        if (response.status === 200) {
            return await {
    ...
    async (id_image) => {
        try {
            const response = await axios.delete(
                `https://itnews-api.herokuapp.com/api/v1/image/${id_image}/db`
            )
            if (response.status === 200) {
                return await {
                    ...response.data,
