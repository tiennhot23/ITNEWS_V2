...
export const changeAccess0 = createAsyncThunk('post/mark', async (id_post) => {
    try {
        const response = await axios.put(`https://itnews-api.herokuapp.com/api/v1/post/${id_post}/status/0`);
...
export const changeAccess1 = createAsyncThunk('post/mark', async (id_post) => {
    try {
        const response = await axios.put(`https://itnews-api.herokuapp.com/api/v1/post/${id_post}/status/1`);
        if (response.status === 200) {
            return await { ...(response.data), status: response.status }
        }
    } catch (error) {
