...
export const loadAuthor = createAsyncThunk("author/fetch", async () => {
    try {
        const response = await axios.get(
            `https://itnews-api.herokuapp.com/api/v1/account/all`
        )
    ...
    async (id_follower) => {
        try {
            const response = await axios.post(
                `https://itnews-api.herokuapp.com/api/v1/follow_account/${id_follower}`
            )
            if (response.status === 200) {
                return await {
                    ...response.data,
    ...
    async (data) => {
        try {
            const response = await axios.put(
                `https://itnews-api.herokuapp.com/api/v1/account/${data.id_account}/role/${data.id_role}`
            )
            if (response.status === 200) {
                return await { ...response.data, status: response.status }
            }
...
export const findAuthorP = createAsyncThunk(
    ...
    async (data) => {
        try {
            const response = await axios.get(
                `https://itnews-api.herokuapp.com/api/v1/account/${data.id_author}/status/${data.id_user}`
            )
            if (response.status === 200) {
                return await { ...response.data, status: response.status }
            }
...
export const limitedTimeLock = createAsyncThunk(
    ...
    async (data) => {
        try {
            const response = await axios.post(
                `https://itnews-api.herokuapp.com/api/v1/account/${data.id_account}/ban`,
                data.lock
            )
            if (response.status === 200) {
                return await {
    ...
    async (data) => {
        ...
        try {
            const response = await axios.patch(
                `https://itnews-api.herokuapp.com/api/v1/account/${data.id_account}/die`,
                { reason }
            )
            if (response.status === 200) {
                return await {
        ...
        builder
            ...
            .addCase(changeRoleAccount.fulfilled, (state, action) => {
                if (action.payload.status === 200) {
                    state.author = state.author.map((author) => {
                        if (
                            author.id_account === action.payload.data.id_account
                        )
                            author.role =
                                action.payload.data.id_role === 2
                                    ? "Moder"
                                    : "User"
                        return author
                    })
